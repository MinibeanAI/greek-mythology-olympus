...
function initAmberCascades(canvas: HTMLCanvasElement) {
  ...
  interface Constellation {
    name: string;
    points: [number, number][]; // 0..1 归一化坐标（相对画布宽与水位上方高度）
    edges: [number, number][];
    phase: number;
  }

  // 神话星座：猎户、天琴、仙后、天鹰——它们都来自希腊神话，在星雨中缓缓浮现
  const CONSTELLATIONS: Constellation[] = [
    {
      name: '猎户座 · Orion',
      points: [[0.62, 0.18], [0.72, 0.14], [0.655, 0.32], [0.665, 0.335], [0.675, 0.35], [0.6, 0.46], [0.735, 0.44]],
      edges: [[0, 2], [1, 4], [2, 3], [3, 4], [2, 5], [4, 6], [5, 6]],
      phase: 0,
    },
    {
      name: '天琴座 · Lyra',
      points: [[0.2, 0.12], [0.225, 0.2], [0.255, 0.185], [0.245, 0.27], [0.21, 0.28]],
      edges: [[0, 1], [1, 2], [1, 4], [2, 3], [3, 4]],
      phase: 2.2,
    },
    {
      name: '天鹰座 · Aquila',
      points: [[0.86, 0.5], [0.9, 0.42], [0.94, 0.52], [0.9, 0.6], [0.83, 0.62], [0.97, 0.62]],
      edges: [[1, 0], [1, 2], [0, 3], [2, 3], [3, 4], [3, 5]],
      phase: 4.4,
    },
  ];
  ...
  function render(timestamp: number) {
    ...
    // 绘制神话星座：星光与连线随呼吸明灭
    const skyH = waterSurface * 0.82;
    for (const c of CONSTELLATIONS) {
      const pulse = 0.5 + 0.5 * Math.sin(time * 0.45 + c.phase);
      const alpha = 0.08 + 0.2 * pulse;
      const pts = c.points.map(([nx, ny]) => ({ x: nx * width, y: ny * skyH + waterSurface * 0.05 }));
      ctx.strokeStyle = `rgba(150, 158, 255, ${alpha * 0.75})`;
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      for (const p of pts) {
        ...
        glow.addColorStop(0, `rgba(205, 210, 255, ${alpha + 0.28})`);
        glow.addColorStop(1, 'rgba(205, 210, 255, 0)');
        ctx.fillStyle = glow;
        ctx.beginPath();
        ctx.arc(p.x, p.y, 7, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = `rgba(240, 244, 255, ${alpha + 0.35})`;
        ctx.beginPath();
      ...
      // 星座名：仅在辉光最盛时浮现
      if (pulse > 0.82) {
        const la = (pulse - 0.82) / 0.18;
        ctx.font = '11px "Fira Code", monospace';
        ctx.textAlign = 'left';
        ctx.fillStyle = `rgba(170, 178, 255, ${la * 0.55})`;
        const anchor = pts[0];
        ctx.fillText(c.name, anchor.x + 10, anchor.y - 8);
    ...
    for (const col of columns) {
      ...
      for (let j = 0; j < col.length; j++) {
        ...
        if (j === 0) { r = 235; g = 238; b = 255; }
        else if (j < 3) { r = 196; g = 182; b = 255; }
        else { r = 126; g = 118; b = 214; }

        ctx.fillStyle = `rgba(${r},${g},${b},${brightness})`;
        if (j === 0) {
          ctx.shadowColor = 'rgba(190, 180, 255, 0.65)';
          ctx.shadowBlur = 8;
        }
    ...
    waterGrad.addColorStop(0, 'rgba(10, 10, 26, 0.6)');
    waterGrad.addColorStop(1, 'rgba(5, 5, 14, 0.95)');
    ...
    ctx.strokeStyle = 'rgba(158, 142, 240, 0.28)';
    ctx.lineWidth = 1.5;
    ctx.stroke();
    ...
    for (const r of ripples) {
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        ctx.strokeStyle = `rgba(158, 142, 240, ${alpha * (1 - ring * 0.3)})`;
